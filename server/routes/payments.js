...
function genReceiptNumber() {
  ...
  const last = db.prepare("SELECT receipt_number FROM payments WHERE receipt_number LIKE ? ORDER BY id DESC LIMIT 1").get(prefix + '%');
...
router.get('/:id', (req, res) => {
  const p = db.prepare(`SELECT p.*, s.name as student_name, s.code as student_code, s.phone as student_phone, s.email as student_email,
...
router.post('/', (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO payments (receipt_number, registration_id, student_id, employee_id, amount, payment_date, payment_method, payer_name, note)
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE payments SET amount=?, payment_date=?, employee_id=? WHERE id=?').run(
