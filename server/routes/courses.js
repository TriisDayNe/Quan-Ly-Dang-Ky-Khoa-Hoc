...
function genCode() {
  const last = db.prepare("SELECT code FROM courses ORDER BY id DESC LIMIT 1").get();
...
router.post('/', adminOnly, (req, res) => {
  ...
  const code = genCode();
  const r = db.prepare('INSERT INTO courses (code, name, description, duration, price, start_date, end_date, status) VALUES (?,?,?,?,?,?,?,?)').run(code, name, description || '', duration || 0, price, start_date || null, end_date || null, 'active');
...
router.put('/:id', adminOnly, (req, res) => {
  ...
  db.prepare('UPDATE courses SET name=?, description=?, duration=?, price=?, start_date=?, end_date=?, status=? WHERE id=?').run(
    name ?? existing.name,
...
router.delete('/:id', adminOnly, (req, res) => {
  const used = db.prepare('SELECT id FROM classes WHERE course_id = ?').get(req.params.id);
