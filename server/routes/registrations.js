...
function genCode() {
  const last = db.prepare("SELECT code FROM registrations ORDER BY id DESC LIMIT 1").get();
...
router.get('/:id', (req, res) => {
  const r = db.prepare(`SELECT r.*, s.name as student_name, s.code as student_code, s.phone as student_phone, s.email as student_email, s.address as student_address,
  ...
  r.payments = db.prepare('SELECT * FROM payments WHERE registration_id = ? ORDER BY payment_date DESC').all(r.id);
  // Parse class_ids and fetch all class details
  if (r.class_ids) {
    const ids = r.class_ids.split(',').map(Number);
    r.all_classes = [];
    for (const cid of ids) {
      const cls = db.prepare(`SELECT c.*, co.name as course_name, co.price as course_price FROM classes c LEFT JOIN courses co ON c.course_id = co.id WHERE c.id = ?`).get(cid);
...
router.post('/', (req, res) => {
  ...
  for (const cid of classList) {
    const cls = db.prepare(`SELECT co.price FROM classes c JOIN courses co ON c.course_id = co.id WHERE c.id = ?`).get(cid);
  ...
  const r = db.prepare(`INSERT INTO registrations (code, student_id, class_id, employee_id, registration_date, status, total_amount, discount, final_amount, note, class_ids)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(
  ...
  for (const cid of classList) {
    db.prepare('UPDATE classes SET current_students = (SELECT COUNT(*) FROM registrations WHERE (class_id = ? OR class_ids LIKE ?) AND status != ?) WHERE id = ?').run(cid, `%${cid}%`, 'cancelled', cid);
  }
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE registrations SET student_id=?, class_id=?, employee_id=?, registration_date=?, status=?, total_amount=?, final_amount=?, class_ids=? WHERE id=?`).run(
...
router.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM payments WHERE registration_id = ?').run(req.params.id);
